...
export async function GET(request: NextRequest) {
  try {
    ...
    const results = searchResponse.data.items.filter((result: any) => result.snippet.description !== '')
		const channelIds = results.map((video: any) => video.snippet.channelId)
		...
		const search = results.map((result: any) => {
